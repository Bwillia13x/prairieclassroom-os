...
- **Needs redeploy/recheck:** the 2026-05-16 pre-submit public QA in `qa/final-release/2026-05-16-pre-submit-e2e/` reconfirmed HTTP 200 root/canonical URLs, root CTA behavior, the `prairie-static-demo-api` marker, static-fallback output labelling, clean browser console/page errors, and route timing evidence. It also found a current deployed text-fit defect: after generating static Differentiate variants, the recent-run chip clipped the artifact title. The source fix is local in `apps/web/src/components/RecentRunsChipRow.css`; redeploy Vercel and rerun `qa/final-release/2026-05-16-pre-submit-e2e/public-demo-check.mjs` before treating the public demo as final-submission ready.
...
- Latest public smoke against `https://prairieclassroom-os.vercel.app` passed with `PASS browser smoke` on 2026-05-16 during the public root/canonical demo check. Earlier 2026-05-13 direct generation saw Render inference 502/timeouts before service recovery, which drove the frontend fallback hardening; the public Vercel route now avoids that cold-start risk by serving the bundled static demo first.
- Latest stricter public browser check in `qa/final-release/2026-05-16-pre-submit-e2e/` confirms the root/canonical route and static-first marker, but the currently deployed Vercel build still has the recent-run chip text-fit issue described above. Public submission readiness now depends on redeploying the local source fix and rerunning the stricter public check.
- Latest current public root/canonical demo check against `https://prairieclassroom-os.vercel.app` wrote fresh capture evidence to `qa/final-release/2026-05-16-public-demo-check/`. The broader deployed route bundle at `qa/final-release/2026-05-15-vercel-submission-final/` predates the first-entry landing page and passed the earlier root redirect behavior plus clean-session first-run, route, generated-output, command-palette, console, bad-response, overflow, mobile/tablet/desktop/wide, protected-route, stale-demo-classroom, static-fallback label, security/header, and Lighthouse assertions. Remaining Lighthouse items are documented as non-blocking P2 metrics, not P0/P1 app failures.
