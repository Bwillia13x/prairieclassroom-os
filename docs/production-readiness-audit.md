...
- **Pass:** 2,086 vitest + 76 pytest cases pass; no skipped schema suites; migration `_migrations` table reflects current schema version.
...
- **Scope:** Seven-view top-level nav (`today / classroom / tomorrow / week / prep / ops / review`), canonical `tab`+`tool` URL state, legacy `?tab=<old-panel>` migration, classroom-code retry flow, 12 teacher-facing panels.
