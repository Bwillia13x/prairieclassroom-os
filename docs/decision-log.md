...
### 2026-04-24 — Design delta closure: extract OperationalPreview primitive, evolve PageHero, introduce command-block surface family

- **Decision:** Close the gap between the live app and the generated design comps as directional product UI. (1) Introduced six new tokens (`--color-command-surface/-rule/-eyebrow/-border`, `--color-preview-surface/-tile/-border`) for a framed command-block aesthetic, scoped to exactly two consumers (`PageHero`, `OperationalPreview`). (2) Evolved `PageHero` to support labeled `metricGroups`, `statusRows`, and tone-aware metric figures without breaking the flat `metrics` API. (3) **Extracted** a new `OperationalPreview` shared primitive (`components/shared/OperationalPreview.tsx`) for the dense below-hero strip of chip rows, evidence rows, and group meta. Preserved the seven-tab nav, per-page role scopes, URL contract, demo data, and model-routed prompt inventory. Commits `b287be2` (polish pass) + `aca6c7b` (HEAD).
- **Why:** The generated comps strongly implied a command-center hierarchy (metric tiles, operational previews, status rows) that the existing page scaffolds could not express without duplicating markup across Classroom/Today/Tomorrow/Week/Ops/Review. `PageHero` already carried the shape language for hero-tier surfaces; extending it (not rebuilding) was the minimum-viable structural change. The preview strip appeared across ≥4 pages with identical shape (section header + compact chip/evidence rows + right-aligned meta), which crossed the threshold for shared-primitive extraction rather than per-page composition. The retrospective plan's stated recommendation was compose-only; the implementer's judgment call to extract is documented under §What would change this and re-validated by the fact that `OperationalPreview` is now used on Classroom, Today, both Tomorrow tool states, and all four Ops tools.
- **Alternatives considered:** **Compose-only (no new primitive):** use existing `OperatingDashboard`, `InterventionRecencyTimeline`, `PendingActionsCard`, `ClassroomCompositionRings`, `TriageSurfaces` differently per page. Rejected because those components are domain-specific (rings for roster composition, timeline for intervention recency) and the preview strip is a cross-cutting shape container, not a domain visualization. **Pixel-perfect comp reproduction:** rejected — comps contained hallucinations (fake student names, alt nav labels, invented metrics) that would violate the product boundaries and demo-fixture contract. **Retaining bespoke `.tomorrow-planning-hub` / `.week-command-hub`:** rejected — the pass replaced both with `PageHero` + tool-aware preview, pruning ~7.7 kB of dead CSS (246.24 → 238.55 kB bundle; gzip 36.07 → 35.38 kB). **Splitting Tomorrow into two separate pages:** rejected — the page+tool-switcher pattern carries deep-link contracts (`?tool=tomorrow-plan` / `?tool=complexity-forecast`) and EA-facing workflows; visual differentiation of the two tool states inside one page was sufficient.
- **Consequences:** Per-page command-block hierarchy now consistent across all seven top-level pages. `OperationalPreview` renders aria-labelled section landmarks; existing zone layouts undisturbed. Duplicate-key warning in `CommandPalette.buildPaletteRows` (root-caused during implementation — not the "header actions" framing in the original brief) fixed with a regression test asserting unique header keys across repeated kinds. Single-row chrome breakpoint moved 1280px → 1660px to avoid tab cramming on common laptop viewports (1280–1659px keep the two-row stacked layout). Validation: typecheck, lint, 1889/1889 vitest, build, contrast 80/80 WCAG AA, live browser sweep across 375/1280/1440/1720 × light/dark — zero console errors, zero duplicate React key warnings. Evidence: `docs/evidence/2026-04-24-design-delta-closure.md` + `output/ui-touchpoints/2026-04-24T20-37-00Z-*` (pre) and `output/ui-touchpoints/2026-04-24T23-47-current-design-delta/` (post with comp reference).
- **What would change this:** Teacher testing showing the command-block aesthetic over-weights the hero area relative to workspace content; a future page requiring a hero pattern that `PageHero` variants cannot express (solve with a new primitive, not a `PageHero` fork); `OperationalPreview` usage shrinking below the four-page threshold on future refactors (would argue for collapsing it back into per-page composition); or a contrast regression the gate misses (e.g. focus-ring visibility on the new tile surfaces) that forces a token-value revision.
