...
- **#4 — Done 2026-04-17 (all 7 retrieval-backed planning-tier routes covered).** `RetrievalTrace` shared schema (`packages/shared/schemas/retrieval-trace.ts`); orchestrator helper (`services/orchestrator/retrieval-trace.ts`) with citation builders for plan, intervention, pattern_report, forecast, scaffold_review, survival_packet, and family_message source types; collapsible `<RetrievalTraceCard>` mounted on TomorrowPlanPanel, EABriefingPanel, SupportPatternsPanel, ForecastPanel, EALoadPanel, and SurvivalPacketPanel. The `detect_scaffold_decay` route emits the same trace on its response, but has no dedicated frontend panel yet — the response contract is defined in `apps/web/src/types.ts` for completeness. Each card shows N "records pulled from classroom memory" with source type, record id, age chip, and excerpt. The honesty-bounded copy reads: *"These are the records the system read into the prompt. The trace reports what was retrieved — not what the model actually used."* Verified end-to-end: `POST /api/tomorrow-plan` against the seeded demo classroom returns 9 citations (3 plans + 5 interventions + 1 pattern report); the pattern is now mirrored across forecast, ea-load, survival-packet, and scaffold-decay responses.
